...
func separateSelfValueFrom(val string) (valFrom *valueFrom, err error) {
	...
	for valIndex := range val {
		if val[valIndex] == ' ' || val[valIndex] == '(' || val[valIndex] == ')' || val[valIndex] == '$' || string(val[valIndex]) == "\"" || string(val[valIndex]) == "'" {
			continue
		}
		...
		for postitionIndex := range argsMap[position] {
			...
			for {
				...
				arg := &Argument{
					...
					Prefix:    argsMap[position][postitionIndex].Prefix,
				}
...
func separateArgValueFrom(stepDAG *Step, arg *Argument, valueFrom string) (valueFromLeft string, err error) {
	...
	// // Case 1: valueFrom is raw string, includes .
	// if signIndex := strings.Index(valueFrom, "$("); signIndex == -1 {
	// 	arg.Prefix = arg.Prefix + " " + valueFrom
	// }
